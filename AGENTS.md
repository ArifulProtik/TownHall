...
- Handlers and services hold no logger. Only three places log: `response.Error` (5xx only, via `slog.Default`), `filestore.Upload` (Warn when UploadThing fails and local fallback is used), and `main.go` startup paths (`log.Error(...)` then `os.Exit(1)` — slog has no Fatal). 4xx are never logged per-request; the access log already records them.
